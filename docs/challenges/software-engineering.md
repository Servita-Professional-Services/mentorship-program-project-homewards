...
Document your classification and use it to justify *where* you place boundaries. A single top-level boundary is not sufficient — it would treat a broken sidebar the same as a broken patient status panel.
...
**Questions to answer:**
- What is the actual cost of `React.memo`? Describe a case where wrapping a component in `memo` makes performance *worse*
...
> **Sprint:** Backlog
>
> *"When I'm reviewing a patient I need somewhere I can write down things that aren't captured in the vital signs. Sometimes the patient mentions something on the phone call, or I want to leave a note for the next clinician covering the ward. At the moment I'm keeping notes in a Teams chat which isn't ideal. Can we add a notes section to the patient page? This would be really helpful for the whole team."*
...
Read the codebase and list every file and system that would need to change to deliver this feature end-to-end. For each, explain *why* it needs to change — not just that it does.
